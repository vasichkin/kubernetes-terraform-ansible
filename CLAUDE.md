...
3. **`ansible-playbooks/`** — run in this dependency order (also encoded in `main_playbook.yml`):
   ...
   - `master.yml`: runs on `k8s_master` — discovers the master's public IP/DNS (via `checkip.amazonaws.com` + `nslookup`), renders `kubeadm-config.yaml` (kubeadm.k8s.io/v1beta4) with the public IP as `controlPlaneEndpoint` and IP/DNS as certSANs, runs `kubeadm init`, sets up `~/.kube/config`, and applies the Flannel CNI manifest. Idempotent via `creates:` sentinel log files.
